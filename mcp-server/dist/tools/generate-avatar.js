...
export function registerGenerateAvatar(server) {
    server.registerTool("q3d_generate_avatar", "根据上传的照片生成 Q 版 2D 形象。支持多种 AI Provider：" +
        ...
        model: {
            type: "string",
            description: "【TRAE 模式可选】指定 Auto Mode 模型进行照片分析和 prompt 优化。" +
        ...
        optimizePrompt: {
            type: "boolean",
            description: "【TRAE 模式可选】是否使用 AI 模型优化图像生成 prompt。" +
                "设为 true 时，会先使用模型根据照片分析结果生成更精准的英文 prompt，" +
                "再用优化后的 prompt 生成图像，通常能获得更好的生成效果。默认 false。",
        },
        for3D: {
            type: "boolean",
            description: "【可选】是否生成 3D-ready 图像。设为 true 时，" +
        ...
        try {
            ...
            let optimizedPromptOverride = undefined;
            if (optimizePrompt && provider.optimizePromptWithModel) {
                const promptModel = model || "auto";
                try {
                    optimizedPromptOverride = await provider.optimizePromptWithModel(analysis, style, promptModel);
                }
                catch (err) {
                    if (err instanceof TraeCollabError &&
