...
import { routeModel, routeModelByComplexity, ModelTask, } from "../providers/model-router.js";
import { buildModelAdaptedScriptPrompt } from "../utils/prompt-optimizer.js";
/** 加载 Pipeline 状态 */
function loadPipelineState(sessionId) {
...
export function registerPipelineGenerate(server) {
    server.registerTool("q3d_pipeline_generate", "一站式 3D 建模 Pipeline 编排器。自动推进 Pipeline 各阶段（照片分析→多视图→脚本→执行→评估→优化），" +
        ...
        try {
            const { sessionId, photoPath, photoAnalysis, generatedImagePaths, scriptPath, qualityAssessment, style, } = args;
            const outputDir = getSessionPath(config.outputDir, sessionId);
            const state = loadPipelineState(sessionId);
            // 更新状态
            if (photoAnalysis)
                state.photoAnalysis = JSON.parse(photoAnalysis);
            if (generatedImagePaths)
                state.multiviewPaths = JSON.parse(generatedImagePaths);
            if (scriptPath)
            ...
            switch (state.currentStage) {
                            ...
                            content: [
                                {
                                    ...
                                    text: JSON.stringify({
                                        ...
                                        message: "请使用 " + visionRoute.modelName + " 模型分析照片，" +
                            ...
                            content: [
                                {
                                    ...
                                    text: JSON.stringify({
                                        ...
                                        message: "请依次调用 GenerateImage 生成三视图，" +
                                            "然后将三个图片路径以 JSON 数组格式作为 generatedImagePaths 参数重新调用此工具。",
                                        hint: "1. 使用 " + complexityAdjusted.modelName + " 模型\n" +
                ...
                case "script": {
                    ...
                    if (!state.scriptPath) {
                        ...
                        const referenceSnippetsPath = path.join(process.cwd(), "bridge", "blender-scripts", "q3d-reference-snippets.py");
...
- 手办级质量`;
                        const modelAdaptedPrompt = buildModelAdaptedScriptPrompt(scriptRoute.modelId, basePrompt, referenceSnippets);
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: JSON.stringify({
                                        ...
                                        message: "请使用 " + scriptRoute.modelName + " 模型生成 Blender Python 脚本，" +
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    message: "Script 就绪！请调用 q3d_execute_blender_script 执行脚本。\n" +
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    message: "请使用 " + assessRoute.modelName + " 模型评估 3D 模型质量，" +
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    message: `第 ${state.refineCount}/${state.maxRefineCount} 次优化。` +
...
//# sourceMappingURL=pipeline-generate.js.map