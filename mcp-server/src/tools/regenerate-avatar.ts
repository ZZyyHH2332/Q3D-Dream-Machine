 ...
 */
export function registerRegenerateAvatar(server: any): void {
  server.registerTool(
    "q3d_regenerate_avatar",
    "形象重新生成 - 基于已有作品换风格、记录重新生成历史（生成操作由 TRAE 的 GenerateImage 完成）",
    {
      workId: {
        type: "string",
    ...
    async (args: {
      ...
      try {
        ...
        let sourceAvatarPath: string | null = null;
              ...
              text: JSON.stringify(
                {
                  ...
                  reasonName: reasonNames[reason] || reason,
                  nextStep:
                    "请使用 GenerateImage 工具生成新形象（使用新风格的 prompt），然后调用 q3d_save_avatar 保存到新的 sessionId",
