...
export function registerGetProjectInfo(server) {
    server.registerTool("q3d_get_project_info", "获取 Q3D 项目信息 - 配置、AI Provider 状态、作品统计、功能清单", {
        infoType: {
            type: "string",
            description: "信息类型：config（配置）/ providers（AI Provider状态）/ stats（作品统计）/ features（功能清单）/ tools（MCP工具列表）/ all（全部）",
        },
    }, async (args) => {
        try {
