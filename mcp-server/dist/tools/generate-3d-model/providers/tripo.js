...
async function uploadImageToTripo(imagePath, apiKey) {
    ...
    const response = await fetch(`${TRIPO_API_BASE}/upload/sts`, {
        ...
        body: (() => {
            ...
            fd.append("file", new Blob([imageBuffer], { type: `image/${contentType}` }), path.basename(imagePath));
            return fd;
        })(),
    });
...
async function createModelTask(imageToken, apiKey) {
...
async function pollTask(taskId, apiKey, timeoutMs = 300000, intervalMs = 5000) {
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        if (["failed", "banned", "expired"].includes(data.status)) {
            throw new Error(`Tripo task ${data.status}: ${data.error || JSON.stringify(data)}`);
        }
        ...
        if (!apiKey) {
            return {
                ...
                error: {
                    ...
                    suggestion: "请设置环境变量 Q3D_TRIPO_API_KEY（获取地址：https://platform.tripo.ai）",
                },
            };
        }
        try {
