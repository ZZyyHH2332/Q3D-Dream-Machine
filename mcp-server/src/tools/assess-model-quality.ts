 ...
 */
async function renderGlbPreviews(
  glbPath: string,
  outputDir: string
): Promise<{ front: string; side: string; back: string } | null> {
  try {
    const renderScript = `
import bpy
import os
...
export function registerAssessModelQuality(server: any): void {
  server.registerTool(
    "q3d_assess_model",
    "评估已生成的 3D 模型质量。通过 Blender Bridge 渲染 GLB 多角度预览图，" +
    ...
    async (args: {
      ...
      try {
        ...
        const prompt = buildQualityAssessmentPrompt(
          assessRoute.modelId,
          referencePaths,
          renderPaths ? [renderPaths.front, renderPaths.side, renderPaths.back] : []
        );
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                prompt,
                message:
                  "请使用 " + assessRoute.modelName + " 模型对比 GLB 渲染图与参考图，" +
