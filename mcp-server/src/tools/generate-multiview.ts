...
import { enhanceFor3D, VIEW_PROMPT_SUFFIX } from "../utils/prompt-3d-ready.js";
import { routeModel, ModelTask, ModelRoute } from "../providers/model-router.js";
 ...
 */
function extractStyleFromAnalysis(analysis: PhotoAnalysis): string {
  // 从 overallVibe 推断风格
 ...
 * 保存多视图图片到会话目录
 */
function saveMultiviewImages(
  uploadId: string,
  imagePaths: { front?: string; side?: string; back?: string },
  style: string,
  photoAnalysis?: PhotoAnalysis
): { multiviewPaths: string[]; metadataPath: string } {
  const outputDir = getSessionPath(config.outputDir, uploadId);
  ensureDir(outputDir);

  const multiviewPaths: string[] = [];
  const viewMap: Array<[keyof typeof imagePaths, string]> = [
...
export function registerGenerateMultiview(server: any): void {
  server.registerTool(
    "q3d_generate_multiview",
    "根据上传的照片生成多视图（正面/侧面/背面）2D 图像，用于高质量 3D 重建。" +
      ...
      "【多模型协作】支持指定 Auto Mode 模型（如 Doubao-Seed-2.1-Pro）进行更精准的照片分析。",
    {
      ...
      model: {
        type: "string",
        description:
          "【TRAE 模式可选】指定 Auto Mode 模型进行照片分析。" +
    ...
    async (args: {
      ...
      try {
        ...
        if (generatedImagePaths) {
          ...
          let paths: { front?: string; side?: string; back?: string };
          try {
          ...
          const { multiviewPaths, metadataPath } = saveMultiviewImages(
            uploadId,
            paths,
            finalStyle,
            photoAnalysisObj
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                message:
                  "请依次调用 GenerateImage 工具生成三张图（正面/侧面/背面），" +
                  "然后将三个图片路径作为 generatedImagePaths 参数（JSON 数组格式）重新调用此工具。",
                hint:
                  "1. 使用 " + imageRoute.modelName + " 模型\n" +
