 ...
 */
export function registerCreateBonesPreview(server: any): void {
  server.registerTool(
    "q3d_create_bones_preview",
    "创建带骨骼动画的 3D 预览页面（支持 9 种动画：7 种心情 + 攀爬 + 倒挂爬行）",
    {
      avatarPath: {
        type: "string",
      ...
      mood: {
        type: "string",
        description:
          "初始心情/动画：idle(平静) happy(开心) excited(兴奋) sleeping(困倦) curious(好奇) sad(难过) love(喜爱) climbing(攀爬) crawling_upside(倒挂)",
      },
      autoRotate: {
        type: "boolean",
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  ...
                  error: {
                    ...
                    message: "未找到 Q 版形象图片",
                    suggestion:
                      "请先调用 q3d_generate_avatar 生成形象，或提供 avatarPath 参数",
                  },
                }),
              },
            ],
