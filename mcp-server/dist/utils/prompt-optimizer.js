 ...
 * 为脚本生成任务构建模型适配的格式指令
 */
export function buildFormatInstructions(modelId) {
    const config = getModelPromptConfig(modelId);
 ...
 */
export function buildModelAdaptedScriptPrompt(modelId, basePrompt, referenceSnippets) {
    const config = getModelPromptConfig(modelId);
 ...
 * 为质量评估任务构建 prompt
 */
export function buildQualityAssessmentPrompt(modelId, referencePaths, renderPaths) {
    const config = getModelPromptConfig(modelId);
    return `${config.systemPrompt}
 ...
 * 为多视图生成任务构建 prompt
 */
export function buildMultiviewPrompt(modelId, viewType, analysis, style) {
    const config = getModelPromptConfig(modelId);
