 ...
 * 支持同时控制 3D 预览页和宠物页面
 */
export function registerControlMood(server) {
    server.registerTool("q3d_control_mood", "心情系统控制 - 设置角色心情/动画、获取当前状态、列出可用心情（共9种：7种心情 + 攀爬 + 倒挂）", {
        action: {
            type: "string",
...
function savePetMood(sessionId, mood) {
    const statusPath = getPetStatusPath(sessionId);
    let status = { currentMood: mood, lastUpdated: Date.now() };
    if (fs.existsSync(statusPath)) {
        try {
...
//# sourceMappingURL=control-mood.js.map