 ...
 * 读取参考代码片段
 */
function loadReferenceSnippets(): string {
...
function buildScriptGenerationPrompt(
  multiviewPaths: { front?: string; side?: string; back?: string },
...
export function registerGenerateBlenderScript(server: any): void {
  server.registerTool(
    "q3d_generate_blender_script",
    "根据多视图图片和角色分析结果，生成 Blender Python 脚本。" +
    ...
    async (args: {
      ...
      try {
        const {
          sessionId,
          multiviewPaths: multiviewPathsStr,
                ...
                text: JSON.stringify(
                  {
                    ...
                    message:
                      "Blender 脚本保存完成！下一步：调用 q3d_execute_blender_script 执行脚本生成 GLB。",
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  ...
                  error: {
                    ...
                    suggestion:
                      "请先调用 q3d_generate_multiview 生成多视图，然后传入 multiviewPaths 和 photoAnalysis。",
                  },
                }),
              },
            ],
        ...
        let multiviewPaths: { front?: string; side?: string; back?: string };
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                glbOutput: glbOutputPath,
                message:
                  "请使用 " + scriptRoute.modelName + " 模型根据 prompt 生成完整的 Blender Python 脚本，" +
