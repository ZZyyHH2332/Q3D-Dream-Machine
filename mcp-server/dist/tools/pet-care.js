...
// 默认宠物状态
function getDefaultPetState(name = "小Q") {
    return {
        name,
        level: 1,
        totalExp: 0,
 ...
 */
export function registerPetCare(server) {
    server.registerTool("q3d_pet_care", "宠物养成系统 - 喂食、玩耍、查看状态、改名、设置性格（200级好感度养成）", {
        action: {
            type: "string",
            description: "操作类型：status（查看状态）/ feed（喂食）/ play（玩耍）/ rename（改名）/ set_personality（设置性格）/ reset（重置）",
            required: true,
        },
        sessionId: {
            type: "string",
...
function savePetState(sessionId, state) {
    const statusPath = getPetStatusPath(sessionId);
...
function formatPetState(state, levelInfo) {
    const personalityNames = {
        friendly: "友好",
        naughty: "调皮",
        lazy: "慵懒",
