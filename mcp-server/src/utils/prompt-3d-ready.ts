...
export const THREE_D_READY_SUFFIX =
  "3D model reference sheet style, centered composition, full body visible from head to feet, " +
...
export const VIEW_PROMPT_SUFFIX: Record<string, string> = {
...
export const THREE_D_NEGATIVE_PROMPT =
  "side view, back view, three-quarter view, turned body, " +
...
// 不同风格的 3D-ready 前缀
const STYLE_3D_PREFIX: Record<string, string> = {
  kawaii:
    "3D chibi character design, cute Q-version figurine, " +
    "big head small body proportion (head:body = 1:1.5), " +
    "round smooth surfaces, soft material appearance, ",
  guofeng:
    "3D Chinese style figurine, elegant Q-version character, " +
    "traditional costume with clear structure, " +
    "smooth ceramic-like surface, ",
  trendy:
    "3D collectible toy figure, blind box style, " +
    "high detail miniature sculpture, " +
    "glossy plastic material, sharp clean edges, ",
  simple:
    "3D minimalist character figure, clean geometric shapes, " +
 ...
 */
export function build3DReadyPrompt(
  style: string,
  analysis: {
    gender: string;
    ageRange: string;
