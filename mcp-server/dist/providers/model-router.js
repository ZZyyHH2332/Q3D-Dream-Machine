 ...
 */
/** 模型任务类型 */
export var ModelTask;
(function (ModelTask) {
    ModelTask["VISION_ANALYSIS"] = "vision_analysis";
    ModelTask["IMAGE_GENERATION"] = "image_generation";
    ModelTask["PROMPT_OPTIMIZATION"] = "prompt_optimization";
    ModelTask["SCRIPT_GENERATION"] = "script_generation";
    ModelTask["SCRIPT_REFINEMENT"] = "script_refinement";
    ModelTask["QUALITY_ASSESSMENT"] = "quality_assessment";
    ModelTask["MULTIVIEW_GENERATION"] = "multiview_generation";
})(ModelTask || (ModelTask = {}));
/**
 * 模型路由表
 */
 ...
 */
export function routeModelByComplexity(task, analysis) {
 ...
 * 估计角色复杂度
 */
function estimateComplexity(analysis) {
    if (!analysis)
        return "medium";
    let score = 0;
    // 配饰数量
    ...
    if (hairDetails.length > 30)
        score += 2;
    else if (hairDetails.length > 15)
        score += 1;
    // 服装复杂度
    if (analysis.outfit?.outerwear)
        score += 1;
    if (analysis.outfit?.material)
        score += 1;
    // 特殊特征
    if (analysis.special_features?.length)
        score += analysis.special_features.length * 0.5;
    if (score >= 4)
        return "high";
    if (score >= 2)
        return "medium";
 ...
 * 供 Pipeline 编排器使用
 */
export function getAllTaskRoutes() {
    const result = {};
