import { getAllWorks, getWorkById, removeWork, getWorksStats, } from "../utils/works-index.js";
/**
 * q3d_manage_gallery
 * 作品画廊管理：查看作品列表、获取作品详情、删除作品、获取统计
 */
export function registerManageGallery(server) {
    server.registerTool("q3d_manage_gallery", "作品画廊管理 - 查看作品列表、获取详情、删除作品、统计数据", {
        action: {
            type: "string",
            description: "操作类型：list（列表）/ get（详情）/ delete（删除）/ stats（统计）",
            required: true,
        },
        workId: {
            type: "string",
        ...
        try {
            ...
            switch (action) {
                case "list": {
                    ...
                    // 按风格筛选
                    if (args.style) {
                        works = works.filter((w) => w.style === args.style);
                    }
                    // 按状态筛选
                    if (args.status) {
                        works = works.filter((w) => w.status === args.status);
                    }
...
//# sourceMappingURL=manage-gallery.js.map