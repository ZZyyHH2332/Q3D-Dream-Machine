...
function validatePromptLength(prompt: string): { valid: boolean; prompt: string; warning?: string } {
...
function mergePrompts(styleDesc: string, descText: string, customPrompt?: string): string {
...
Be specific about colors. Keep each field under 30 words.`,
  ...
  let processedCustomPrompt: string | undefined = undefined;
  if (customPrompt) {
