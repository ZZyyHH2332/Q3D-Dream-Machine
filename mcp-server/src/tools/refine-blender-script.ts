 ...
 * 构建脚本优化 Prompt
 */
function buildScriptRefinePrompt(
...
export function registerRefineBlenderScript(server: any): void {
  server.registerTool(
    "q3d_refine_blender_script",
    "根据用户反馈或错误日志，迭代优化 Blender Python 脚本。" +
    ...
    async (args: {
      ...
      try {
                ...
                text: JSON.stringify(
                  {
                    ...
                    feedback,
                    message:
                      "脚本已更新！下一步：调用 q3d_execute_blender_script 重新执行脚本。",
        ...
        // 构建 prompt
        const prompt = buildScriptRefinePrompt(
          currentScript,
          feedback,
          errorLog,
          referenceSnippets
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                glbOutput: glbOutputPath,
                message:
                  "请使用 " + refineRoute.modelName + " 模型根据 prompt 修改脚本，" +
                  "然后将修改后的脚本保存为文件，将路径作为 scriptPath 参数重新调用此工具。",
                hint:
                  "1. 使用 " + refineRoute.modelName + " 模型（" + refineRoute.reasoning + "）\n" +
                  "2. 根据 prompt 修改脚本\n" +
                  "3. 将脚本保存到文件\n" +
